...
- Ledger writes are CAS (77e485bb23): state/coord/tasks.json carries a per-save `rev`; save() compares it under tasks.json.lock (5s wait) and refuses with LedgerConflict when a peer wrote first -- re-read and re-apply (the conductor door does, 4x); readers never lock; an unreadable anchor fails CLOSED, never paves over damage.
