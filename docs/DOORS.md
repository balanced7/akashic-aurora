...
| `eye` | THE EYE: the transcript plane as terrain -- ingest (incremental, coverage-honest), find (phrase, S0; the grammar lands S1), get (address -> verbatim L0) | `<eye_cmd>* {ingest,find,get,freq,stats,overview,zoom,trace,look,go,back,since,inherit}` |
