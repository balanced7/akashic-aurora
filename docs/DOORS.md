...
| `eye` | THE EYE: the transcript plane as terrain -- ingest (incremental, coverage-honest), find (phrase, S0; the grammar lands S1), get (address -> verbatim L0) | `<eye_cmd>* {ingest,find,get,freq,stats,overview,zoom,trace,standing,look,go,back,since,inherit}` |
...
| `resident` | callsign ceremony: nominate / ratify / show a resident's designation | `<sub> {nominate,ratify,show,assign,place,roster,roles}` |
