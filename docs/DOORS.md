...
| `find` | find a file BY NAME anywhere on the machine (Search Everything / es.exe) | `<query>*` `--limit` `--offset` `--path` `--no-sort` `--timeout` `--sort` `--columns` `--format {,json}` `--regex` `--case` `--word` `--dirs` `--files` `--scope` `--attrs` |
