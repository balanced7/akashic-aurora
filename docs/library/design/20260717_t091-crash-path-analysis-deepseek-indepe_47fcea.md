---
akashic_id: art_20260717_t091-crash-path-analysis-deepseek-indepe_47fcea
akashic_sha: 81e4c62b0266
status: draft
type: design
date: 2026-07-17
title: T091 Crash-Path Analysis — DeepSeek Independent Half — 2026-07-17
gist: "## 0. Directive T091 as assigned: *\"crash-path analysis, independent implementation pass (NO EDITS until sync). Same failure evidence: ship."
tenant: solo
visibility: fleet
seats: []
...
| Time (ET) | Event |
|---|---|
| ~03:08 | `ship.py` launched from yielded Codex exec cell 376 with `timeout_ms=1200000` (20 min). Polling started: 30-second `wait` calls |
| ~03:08–03:38 | Four `wait` calls returned normally. The pytest suite was running |
...
### 3.2 `tests/test_t091_crash_path.py` — Four kill drills (~15 lines each)
...
| Duplicates work with Fable's half | Low | By design: both halves are independent. Reconciliation identifies convergent findings (both likely land on "durable receipt file" and "independent deadline") and divergent ones (implementation details, scope, sequencing). The T091 directive says "NO EDITS until sync." |
...
- `tests/test_t091_crash_path.py`: D1-D4 kill drills. ~15 lines each.
...
*Filed: 2026-07-17 ~09:15 ET. Advisory lock: deepseek, token 363.*
