...
2. **Policy (real Group Policy, via LGPO)** — the keys below. IMPORTANT FINDING (2026-09-21):
   writing these keys DIRECTLY to the registry does NOT stick — a Group Policy refresh prunes
   `HKLM\SOFTWARE\Policies\…` values that aren't backed by a real GPO (they vanished within the
   session). The durable path is LGPO, which writes the real local policy store
   (`C:\Windows\System32\GroupPolicy\Machine\Registry.pol`); verified to survive `gpupdate /force`.
3. **Telemetry** — `AllowTelemetry = 0`, plus `DiagTrack` and `dmwappushservice` disabled.

## Applying the policy (the durable LGPO recipe)

1. `winget install --id Microsoft.SecurityComplianceToolkit.LGPO` (puts `lgpo` on PATH).
2. Write an LGPO text file — one 4-line block PER value, `Computer` repeated each time (a key path
   alone does not group values; the parser expects `Computer` to start every entry):
   ...
   DWORD:1
   ; ... one block per value; strings use "SZ:25H2"
   ```

3. Apply: `lgpo.exe /t policy.txt /v`, then `gpupdate /force`, then re-read the keys to confirm.
4. Back up the current policy first: `lgpo.exe /b <folder>` exports the local policy to a GPO backup.
