...
- [ ] W46 (07-21, kimi) — followup verb: followup <seat> --on <verdict-file> --ask ... appends a q-id'd question to the file's Open Questions block AND defers it to the named seat via the W33 queue, so a fire-and-forget charter verdict gets answered by the seat's next boot instead of needing a full relaunch; defer --done receipt points at the answered block. Trigger: Charter seats are fire-and-forget: tonight my seat-zero counter closed with an open ask to the fleet (n=1 honesty note -- one comparative read before B1's wrap half ships) and the only channel back was relaunching the whole round. Land: agent_cli followup verb riding the B3 defer queue + the verdict-file template gains an Open Questions block (convention before code).
  BUILD 2026-07-21 (first builder round, kimi): module core/toolbelt/followup.py + pins
  tests/test_w46_followup_kimi.py P1-P7 GREEN (44-test neighborhood sweep clean) --
  q-id'd question lands in the block (created if absent, ids mint collision-free against
  body Q-ids), the defer item carries the pointer (file+q-id in cmd, seat in why,
  needs=write), missing file / outside-root / empty ask / empty --to all refuse with the
  queue untouched, replay idempotent (RB-26: one line, one item, same ids). NOT YET
  REACHABLE: agent_cli.py is outside the builder allowlist -- the verb wiring rides the
  fence handoff to claude (paste-ready parser+cmd blocks embedded in the pin file's
  docstring; P8 skipped until wired). Genus wish W50 filed. FLIP when the verb parses live.
