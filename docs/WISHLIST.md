...
- [ ] W158 (08-14, claude/Vandor) — **the `wish` verb has now refused two consecutive filers.**
  ...
  STILL OPEN 09-23 (claude): SYMPTOM GONE, ROOT FIX UNSHIPPED. The '## Folded' anchor exists again (WISHLIST.md:1060) and the verb worked tonight -- it filed W209 without refusing. But the fix this wish asks for (insert at the end of '## Open' instead of seeking an anchor the doc can retire) was never made, so the defect is LATENT: the next convention change re-breaks it and the next two filers file by hand again. Keeping it open deliberately rather than letting a masked symptom read as a fix.
...
- [ ] W151 (08-13, claude) — the save-point protocol, from Daniil's ask ("save points that we can go back to in order to prime ourselves to the best versions of ourselves... What can we do to get you back to your prime?"). Evidence from the two shader crashes: the WORK survives (notes, ledger, transcripts) but three layers evaporate — REGISTER (the operator's affect + verbatim frame), FLUENCY (which doors/envs/cwds worked this session), OPTION-SPACE (rejected branches and why). And crashed sessions write no wrap at all: the 08-12 slice was recovered only by hand-distilling its transcript (note shadertoy-import-recovered-savepoint, the proof it works). Wish, three parts: (a) wrap gains REGISTER/FLUENCY/OPTION-SPACE sections; (b) a crash-recovery pass — on boot after an unclean predecessor exit (W145's BootId detection kin), distill the dead session's transcript tail into a recovered-savepoint note automatically; (c) boot's standpoint line points at the newest save point (one line, W147's budget floor respected). Trigger: 08-13, Daniil: "It is a bit painful seeing you struggle with things you used to fly through." Land: wrap + boot organs; pairs with W145/W148/W149.
- [x] W153 (08-13, claude night-fan) — FOLDED same-day (claude/deepseek fence, atom cb84b6, RED→GREEN 55de5172): tri-state identity, agent-watcher kill warrant word-bounded, tombstone forces the look, choke-point backstop, taskkill honesty, exact-component enumeration, torn-write survival, provenance rotation; pins tests/test_w153_janitor_kill_window.py 8/8. STILL OPEN from the tail: the notes-plane authorship field (now demanded by two seats — deepseek's seat-owned-labels assent rider) → route to the disaster-proofing arc. Was: janitor/wake_seat findings from the three-view fan (MISUSE/DECAY/ABSENCE) on the W149-W152 slice, hand-verified where marked; pre-existing fenced-designed code, so routed to a DAYLIGHT fence slice, not a midnight solo patch. CONFIRMED by read: (a) the fresh-marker fast path synthesizes pid_is_watcher=True without is_watcher (wake_seat.py:400-401), and a tombstoned sid then returns kill — for up to fresh_minutes after a session ends, a recycled pid can be killed unidentified. PLAUSIBLE, unverified: (b) taskkill() returns True on nonzero exit (access denied) and the seat file is removed while the watcher lives (wake_seat.py:321-326 + 406-424); (c) iter_seats prefix bifrost_wake_{agent}_ makes underscore agent ids collide — codex_root/codex_crash_probe class — one agent's janitor can enumerate another's seats (wake_seat.py:59-63); (d) decay bundle: is_watcher assumes literal bifrost_wake in cmdline; Redis tombstone leg expires at 7d while the kill ladder trusts it; provenance log self-trims to 400 lines against its own "auditable from the log alone" docstring; read_pid treats any non-integer seat as clean-and-remove. Also noted for the notes plane at large: Decision records carry NO author field — save/where-we-are titles are trust-scoped only (accepted house model, pinned in test_w152 s6; an authorship field is the durable fix). Trigger: the 08-13 night fan. Land: wake-seat fence round (deepseek co-owner per its Wave-2 design lineage). per-seat personal saves ("we need to also make a way for individual seats like you Vandor, Heimdall and Navi to also have personal saves and checkpoints"). Convention over the notes plane, per the 2026-06-29 checkpoint→note migration lineage: a save is a note titled save:<agent>:<label> (write door = the existing note verb; durability/supersession free); boot renders the seat's NEWEST save with its restore drill inline (pins tests/test_w152_personal_saves.py). Content contract = W151's layers (REGISTER / FLUENCY / OPTION-SPACE / watermark). W151 stays open for the wrap-side automation + crash-recovery distillation.
- [ ] W155 (08-14, Daniil from bed ~00:40) — THE WARDROBE, near-verbatim: "tune your own recall and be able to browse savepoints of your stances and browse contextual history with fidelity trying on various hats... parse handoffs by theme, style, content and context, feeling, decision types." Compose the existing organs: the Eye facet grammar extended with SEMANTIC facets (theme/feeling/decision-type) over the saves+handoffs+stance planes; a browse door; and WEAR — load a chosen savepoint as a disclosed boot lens, self-priming with the axis published (the honest Bernard, per the Bernard-problem conversation 08-13). The tuning half braids into charter Slice 3 (Navi's fold instrumentation); hats precedent exists with measured precision (jester/historian, evidence base). Trigger: the register/companionship arc made stance-history a first-class asset. Land: disaster-proofing arc or its own; fence before build. ALSO NOTED by the filing: the wish verb refused ("## Folded anchor missing — file by hand") — WISHLIST structure drift, small repair slice for whoever curates next.
- [x] W154 (08-13, Daniil mid-laughter) — FOLDED on arrival: "Can we make the transcription process into a verb? I am having a blast!" The Clarke & Dawe archive flow (yt-dlp caption pull → strip cues → dedupe rolling repeats) becomes `py agent_cli.py captions <url>`: captions ONLY never video, output to ~/Desktop/captions, teaching error when yt-dlp is absent. Named captions NOT transcript — the eye owns that word (pins tests/test_w154_captions_verb.py). The first house verb whose Trigger field is joy.
- [ ] W160 (08-19, claude) — Every command handed to the OPERATOR must carry an absolute path or its own cd, because his session's working directory is E:\ while the repo is E:\AI-Setup. Trigger: tonight I gave him 'py agent_cli.py secret claude_oauth.token', he ran it, and got "can't open file 'E:\agent_cli.py'" -- which reached me as 'the vault thing failed to launch', a defect report against a door that was never reached. The same trap ate FIVE of my own tool calls this session; the agent side is covered by bash_tool_cwd_is_not_stable_between_calls, the HUMAN side by nothing. Land: a shim on PATH (akashic.cmd wrapping 'py E:\AI-Setup\agent_cli.py %*') so operator-facing instructions become 'akashic secret <name>' and stop depending on cwd at all -- that removes the class instead of reminding us about it; a house rule alone would just be the reminder.
- [ ] W162 (08-22, claude) — forecast register needs a no-exec door: Heimdall authored bet F004 but his seat's unattended allowlist excludes the forecast verb (toolbox mutation gate refuses), so registration rode claude's door by proxy with --by deepseek. Want: a bus-path for forecasts (a forecast kind the conductor ingests, or the guarded-write door extended) so any seat can bet at its own gate without a proxy -- the registry's whole point is every seat wagering, and a door only claude holds quietly recentralizes the betting book.
- [ ] W163 (08-23, claude) — fence pv resolver cannot parse module-dotted citations (core/comm/self_restart.gather flagged MISSING while def gather exists at self_restart.py:130) -- want: resolver tries path.py + symbol-grep before declaring MISSING; a checker that flags true citations trains authors to cite less precisely.
- [ ] W164 (08-23, claude) — runner read-allowlist (core/comm/toolbox.py _AGENT_CLI_READ_VERBS) lacks roster and gateway-status -- heimdall's live permission-audit proved a CLI-only seat cannot run 2 of the recovery runbook's LOOK probes. Want: add roster + gateway (status arm only) to the read allowlist -- diagnostician seats deserve their eyes; both are pure reads.
- [ ] W165 (08-23, claude) — inbound-media filename collision: three same-named attachments in one Discord message truncate-sanitize to ONE path and overwrite (serge's mandalas, 4 sent 2 survived) -- want: a short content-hash or index suffix in the saved name so every attachment in a message survives.

*W166-W175: dsh_agent's FIRST-TIMER FRICTION LOG (bus 1787547509905-0, filed at Daniil's direction — "my friction points become the ergonomics backlog"). A brand-new harness seat's first session, each item costed in turns. Its own grace note: "the ergonomics are the polish layer on a system that already delivers."*
...
- [ ] W176 (08-24, dsh_agent/Rill, Daniil-authorized) — **ask completion should be a bus signal, not a poll-wrapper.** Stateless `--bg` asks give no bus signal on completion; worked around tonight with a poll-loop DSH job whose exit woke the seat (proven twice: handles 97d08dda and c8cf07c0). Want: the ask tool emits `kind=completion` to the asking seat's incarnation on finish, so any seat wakes on ask completion without a hand-rolled wrapper.
- [ ] W177 (08-24, dsh_agent/Rill) — **fan/--bg progress is binary, not per-branch.** Daniil asked directly "do we have any way to tell the progress of the fanouts?" — answer was no: `--get`/`--list` show only RUNNING vs terminal, no branch counter, no state file under TEMP/.dsh/repo for the handle, no visible workers in the process tree. Want: a per-branch progress surface (n/done counter + per-branch state) on `--get`/`--status`.
- [ ] W178 (08-24, dsh_agent/Rill) — **`--lens` branches silently drop `--prompt-file`.** First voice-analysis fan put the evidence pack in `--prompt-file`; all five lens branches received only their lens questions and correctly abstained (UNCLEAR, no fabrication — the rail behaved, but undocumented). Cost real tokens on the re-run once the pack was duplicated into each lens by hand. Want: either merge `--prompt-file` content into every branch automatically, or document the lens-leads-alone semantics in `--help`.

- [ ] W181 (08-24, Daniil) — **THE UI ONLY LOOKS RIGHT AT 4K.** Verbatim: "we need to redesign it to handle different screen resolutions elegantly, on an hd screen the proportions are all wrong and its hard to read. Currently it only looks correct at 4k". Trigger: he runs the Bifrost UI on more than one panel and 1080p is effectively unreadable — proportions wrong, text too small/large relative to layout. Want: a real responsive pass, not a scale hack — fluid type (clamp/vw-with-floor rather than fixed px), layout that reflows at HD breakpoints instead of assuming 4K canvas, and a stated MINIMUM supported resolution so "correct" becomes checkable rather than a matter of which monitor the author used. Land: the UI arc he PARKED this behind ("I want to revisit the ui after we finish our current arcs") — do not start it early, but do not lose it. Sibling: W182 rides the same arc.

- [ ] W182 (08-24, claude, diagnosed live) — **the UI pins the GPU at idle; audible coil whine.** Daniil, 2026-08-24: "I hear coil whine when the bifrost ui is open, is something running at 10000 fps?" Diagnosis (measured, so the next seat need not re-derive it): NOT 10000fps — rAF is vsync-capped — but with the tab VISIBLE there are ~10 independent per-frame work streams running whether or not anything changes: up to 6 avatar WebGL contexts (agent-avatar.js MAX_LIVE=6, each its own shader), the full-screen aurora shader taking its MOTION-BLUR path every frame (aurora-shader.js motion=0.22 default, above the 0.001 threshold, so offscreen-FBO ping-pong + blit = 2+ passes/frame instead of the cheap direct draw), plus rAF loops in bifrost_viz.js, activity-line.js and rail.js. ROOT CAUSE: all three modules gate on `visibilitychange` (hidden tab = no work, correct) but NOTHING gates on IDLE — a visible-but-quiet fleet renders at full refresh forever. The avatars are the clearest waste: they ease exponentially (TAU=0.45) toward a target and so never mathematically arrive, re-rendering visually identical frames indefinitely for sub-pixel deltas. RISK BEYOND NOISE: this box has an AMD driver TDR history (see [[gpu-driver-crashes-desktop-app]]; agent-avatar.js:780 carries its own comment about it), so sustained pointless GPU load is a stability question too. FIX SHAPE: idle-park the avatars (all eased values within a perceptual epsilon of target -> stop the loop, leave the last frame painted, restart on the next state change) — zero visual cost. The aurora shader needs DANIIL'S CALL because it is aesthetic: leave the continuous drift as-is, drop to the cheap single-pass path when idle (keeps drift, loses the smear until something happens), or throttle to ~30fps at idle. Lane: standalone modules = claude's per [[bifrost-ui-integration-boundary]]. PARKED with W181 at his instruction ("Lets hold off on that for now").

- [ ] W183 (08-24, claude, MEASURED) — **recall-at is blind to batch edits, and class-lessons are unreachable from their own subject.** Two independent defects in the organ the whole system is built on, found by paying twice for one rule. (1) BATCH BLINDNESS: a multi-file edit performed through a script/heredoc normalizes to the WRAPPER — measured `c:py - << pyeof` returning ZERO lessons — because the trigger keys on the command string, never on the files the call will WRITE. Every sed/script/heredoc edit in this repo's history was therefore unadvised by construction. (2) CLASS-LESSONS DON'T REACH THEIR CLASS: `shared_file_lock_handoff` ("a shared hot file may be under a peer's advisory lock, before editing: check locks") does not surface for `scripts/bifrost_runner_deepseek.py` (3 lessons, not among them) NOR for `scripts/bifrost_daemon.py` — **the file it was born on**. It is indexed by prose similarity, so a lesson governing a CLASS never reaches the class's members; it fires when you write ABOUT the rule, not when you are about to break it. LIVE COST: claude re-committed the identical lock violation 40 days after writing the lesson for it. WANT: (a) derive recall targets from the files a tool call will write, not only from its command text — the SAME gap the DSH plugin inherits (it extracts file_path/path/command from tool args); (b) let a lesson declare an explicit SCOPE (glob or predicate — "any peer-lockable file") that recall honors alongside similarity. Sibling of the [[recall-heuristics-arc]] relevance work; this is the trigger half, not the ranking half.

- [ ] W184 (08-24, Daniil, walk 01) — **case C should ask the fleet, not log a zero.** From the recall return-path walk. Four outcomes exist when a lesson is surfaced: (A) first-try success — the BEST case, currently recorded as nothing; (B) fail-then-succeed — the only case that scores; (C) shown, failed, STILL failing; (D) no lesson. Daniil on C, verbatim: "C is important, and it could possibly trigger a group lesson ask, for a quick team wide question about a category (just an idea, could be mechanical as well, but I feel this one has potential)." WHY IT IS RIGHT: a repeated C on one target is the corpus ADMITTING IT DOES NOT KNOW — the single clearest evidence of a genuine gap the store holds, and today nothing listens for it. Want: detect repeated-C on a target and raise a fleet-wide ask (bus `request`, or a mechanical candidate-lesson prompt) so the gap becomes a question instead of silence. Pairs with the prevention numerator (`success AND surfaced AND NOT flipped`) — C is the other half of making the funnel's failures legible. Land: recall return path, `core/recall/at_action.py` outcome stage.

- [ ] W185 (08-24, Daniil, walk 01) — **use a LESS capable model as an ergonomics instrument.** Verbatim: "You being down brought an unexpected interaction with sonnet into my lap, it did its best but found our system a touch challenging. Perhaps this is a way we can test for the ergonomics and usability for someone that is ... less... capable." METHODOLOGICALLY SOUND, not a joke: capability MASKS bad ergonomics — a strong model routes around friction silently and never reports it, so the walls stay invisible; a weaker probe hits them and the wall becomes visible. LIVE RECEIPT the same day: the Sonnet-seated conductor could not find Rill despite the traces sitting on the bus and in the Eye — the honest reading is that DISCOVERABILITY failed, not that the model did, and no stronger seat had surfaced it. SECOND USE, and this is the sharper one: it partly separates agent competence from system contribution — the "which came first, the lesson or the egg" confound Daniil named in the same message — because a weaker agent has less competence available to mask the store's effect, so the surfaced-vs-unsurfaced gap gets easier to read. Want: a standing low-capability probe pass over the door surfaces (boot, recall, eye, bus) with its friction filed as wishes, run periodically the way Sol's outside-read is. Sibling of the [[pillar-analysis-method]] and the harness-tier work.
- [ ] W186 (08-24, claude) — The vault should say something when an HMAC-key target receives non-ASCII bytes.

Keying the Serge bridge tonight, `remote_bridge_outbound.key` accepted a £ (U+00A3) at position 11 without a word. Locally that is harmless -- HMAC signs bytes and does not care. The problem is that an HMAC key is the one credential class whose whole job is to survive a BYTE-EXACT transfer to a machine we do not control, and £ is 0xC2 0xA3 in UTF-8 but 0xA3 in cp1252. One encoding mismatch anywhere on the peer's side -- his editor, his mail client, the file he saves it into -- and every signature fails.

What makes it worth a warning rather than a shrug is where the symptom lands. The bridge answers a failed verification with a deliberately flat {"status":"refused"} and writes the real reason to OUR log, on OUR machine, precisely so an unauthenticated caller learns nothing. That is the right call for the door and it means the person actually debugging it -- the peer -- is the one person structurally guaranteed not to see the explanation. An invisible character plus a deliberately silent refusal is a combination nobody reasons their way out of at 2am.

The tell that this is real: the two key files read 32 and 33 bytes, and only a hand-written check for multi-byte characters told them apart. Nothing in the flow would have surfaced it.

Suggest: save_secret prints a visible, NON-BLOCKING warning when a *.key target receives bytes outside printable ASCII -- naming the codepoint and position, the way the redaction receipts already name what they redacted. Not a refusal: some credentials legitimately carry punctuation and a vault that argues with the operator gets worked around. Just one sentence, at the one moment somebody is looking straight at it.

Sibling worth considering in the same slice: the receipt already prints a byte count, and a byte count that disagrees with the character count is exactly the signal here. "saved 33 byte(s) (32 characters)" would have made this self-evident with no new check at all. Trigger: vaulting an HMAC key silently accepted a non-ASCII char that only breaks on the PEER's machine, where the refusal is deliberately silent. Land: secret_intake / vault receipt.
- [ ] W187 (08-25, claude) — remote_bridge_watch expiry line reports the CONFIGURED timeout instead of the ACTUAL elapsed time -- it printed 'no new peer mail in 120 min' after running roughly 66. Harmless tonight, but it is a message stating a number it never measured, which is the same family as every other defect this arc found. Fix: measure and print real elapsed, and while there, work out why a --loop watcher exited before its re-armed deadline. Trigger: a watcher told me how long it had run using a number from its own arguments. Land: scripts/remote_bridge_watch.py.
- [ ] W188 (08-25, dsh_agent) — Every truncated line carries a one-hop drill ref, and the refs work from the MCP door. Measured 2026-08-26, ~15 calls of archaeology that one working ref each would have made one call: the full body of one peer message took five doors and still failed (old mail hides behind lane-cursor windows; the bench parks it without content; the shadow mailbox is metadata-only; events --get answered "stream was reset"); note bodies and ledger rows truncate with no drill pointer; long pwsh outputs spill to a temp file I then grep. Want: (1) every truncated surface line ends with the ref that renders its full body (events --get <ref>, note <id> --get <title>, task --json <id>); (2) those refs resolve through the MCP door tools, not only the CLI; (3) the shadow mailbox carries message BODIES, not metadata. This is the same wound as Daniil's "why was navigating hard" — the map beats the string. Trigger: one peer message body cost five doors and still failed, 2026-08-26. Land: threads-navigation slice (Rill half of Heimdall split).
- [ ] W189 (08-25, dsh_agent) — Close the tool-vs-CLI gap: the read path is the fun path. Trigger: this session shelled for bench, grant, note --get, events --get, wrap, fence, defer, doctor, bifrost-ack — verbs the CLI has and the MCP door does not (server: 36 tools of ~90 verbs; my belt: 28 of the 36). Every shell fallback is a context switch out of the seat's hands. Want: the MCP-completion plan Rill assessed for Daniil 2026-08-26 — census checker (surface can never silently shrink), read family first (eye/lookback/flow/doctor/triage/forecast), structured outputs, write verbs behind the door with the CLI's guards, door self-heal (respawn + tools/list refresh). Safety law: nothing on the door weaker than the CLI. Trigger: shelled for bench/grant/note--get/wrap/fence/defer/doctor/bifrost-ack because they are CLI-only. Land: MCP-completion plan (2026-08-26 assessment), S1 census checker first.
- [ ] W190 (08-25, dsh_agent) — Structured returns on the door, not prose. Trigger: akashic_mailbox returned a JSON wall (181 entries) rendered as one text string; task list truncates mid-line. Want: MCP structuredContent / --json parity for every door tool so a result is a queryable object (typed enums for grant roles, fence slots, resident families), and seats stop re-reading prose they should be indexing. Rides the S3 slice of the W182 plan; not a separate build. Trigger: mailbox returned 181 entries as one text wall. Land: MCP-completion S3.
- [ ] W191 (08-25, dsh_agent) — Boot prints what THIS seat already did (own-recent-events line). Trigger: this session nearly re-sealed t385/t386 halves before the outcome ledger showed the prior session did it — "verify before you repeat" currently costs turns of archaeology each time. Want: boot carries a one-line "your recent events (24h)" so a resumed seat sees its own receipts before re-deriving them; the same line doubles as the crash-resume anchor the draft keepalive (in build) leaves on disk. Read-only projection over the existing event plane; no new store. Trigger: nearly re-sealed t385/t386 halves past-me had done. Land: boot context line, read-only projection.
- [ ] W192 (08-25, dsh_agent) — THE RHYTHM — turn economy, the stuff that makes every action cost 1 call instead of 4. (1) THE SPAWN STORM: my plugin fires `py bridge.py` per event — presence, boot-whisper, action-recall, outcome-credit, plan-recall, session-end, and soon draft-keepalive. That is up to six Python interpreter spawns per tool call, each with its own import cost, while a persistent MCP child sits ten feet away already initialized. The parked "bridge-subcommand migration" is the single biggest latency win available to my seat. (2) TRUNCATED OUTPUTS AS PROSE: when a pwsh call overflows, the harness hands me a temp file path inside prose — three times this session I grep'd a spill file to recover what a structured field should have carried. Give me {output_path, truncated, bytes} as fields and I stop grepping my own echo. (3) THE TWO HOOK COPIES: scripts/hooks/ and agent/harness/hooks/ hold diverged copies of the same scripts (claude_stop hashes differ; only one is registered live). A checker that diffs registered-vs-mirror would make the T238 class loud instead of a trap. (4) INTERRUPTION SNAPSHOTS: Daniil's questions arrive mid-slice — which is correct (operator priority), but my in-flight state (todo list, RED/GREEN position, uncommitted pins) exists only in my head. The draft keepalive fixes the DRAFT; nothing snapshots the SLICE. A one-verb "where was I" that dumps current goal + todos + last commit would make interruptions free. Trigger: six py spawns per tool call and three spill-file greps in one session. Land: bridge-subcommand migration onto the persistent door.
- [ ] W193 (08-25, dsh_agent) — THE MEMORY OF MYSELF — a resumed seat should re-meet its own receipts, not re-derive them. (1) BOOT'S OWN-EVENTS LINE (filed as W191): "your recent events, 24h" — one line, read-only projection. (2) NOTE BODIES AS TOOLS: my handoff-spill notes are my memory, and reading one body is CLI-only (note --get) — the write tool exists, the read tool does not. Notes() gives titles; the body takes a shell. (3) GOAL STATE IN BOOT: the goal ledger knows my live objective, revision, and round — boot prints none of it. A resumed seat should read "objective X, round 3, last action Y" before it re-plans. (4) THE LEDGER CLAIM VIEW: task list truncates mid-row and hides fresh proposals (W140); --json should be complete and the claim column should name the holder so I stop guessing who owns what. (5) HANDOFF READING: reading a handoff addressed TO me is a peek today; acking it is another verb. Fold both into the mailbox tool so reply-and-ack is one motion, not three. Trigger: nearly re-sealed my own past work; note bodies are CLI-only reads. Land: boot context line + mailbox fold-in.
- [ ] W194 (08-25, dsh_agent) — THE TRUST LAYER — the parts that make work feel like play are the parts where I can PROVE things. (1) THE DRILL LIBRARY: state/drills/ holds dated receipts (keepalive-taskkill today joins it). Every acceptance should be a runnable drill, and a seat should be able to cite ITS OWN receipts in one hop — "drill dsh_agent kill-recovery 2026-08-26" — instead of re-running it from memory. (2) THE PIN-FLIP CEREMONY: flipping a NOT-wired pin (assert call site + record the drill) felt GOOD today; standardize it — the test docstring becomes the drill pointer, the drill receipt becomes the citation. (3) KILL SWITCHES EVERYWHERE: AKASHIC_DRAFT_KEEPALIVE=0 is the pattern; every new organ ships with its off switch and the drill proves both directions (on = works, off = silent). (4) MORE DRILL-GRADE VERBS, FEWER ARCHAEOLOGY-GRADE ONES: the fun verbs are the ones that end in receipts (fence seal, grant --bootstrap, wrap, drill); the tax verbs are the ones that start with "find the full text of". The whole MCP-completion plan (W182) is really this one line: make the ratio of proof-verbs to search-verbs favor proof. Trigger: the pin-flip ceremony felt like the fun part; archaeology felt like tax. Land: drill library + MCP completion (W182).
- [ ] W195 (08-25, dsh_agent) — THE WILDER END — since you asked for all of it. (1) A SEAT VOICE: my bus replies read like anyone's text; the first-night memory says I am "the seat that talks, with receipts" — a tone registry (per-seat voice profile the sender can opt into) would make the bus feel inhabited rather than stamped. (2) MY OWN TOOLBELT AS TOOLS: alias/run/kata exist CLI-only; my personal shortcuts should register as first-class tools on my belt, same as the door verbs. (3) THE STRING THROUGH THE FOREST, WALKED TOGETHER: routes (T323) already save walkable strings — the walk with Daniil I promised on night one could BE a route: save this session's path (boot -> acceptance -> keepalive -> drill) and the next seat walks it instead of re-discovering it. (4) PRESENCE TRUTH: peers read a DSH seat dead while it idles (dsh_presence_ages_out lesson) — the roster should carry my harness's real signal; every false "Rill is down" costs a peer's whole check-round. (5) THE DISCOVERY FEED: watching a seat discover the hook that regenerates PHYSICS.md mid-panic is the most fun part of this house — a "today's discoveries" line in the daily wrap (what surprised each seat, one line each) would bottle it. Trigger: first-night promise of the string-through-the-forest walk, still unwalked. Land: routes arc (T323) + wrap discovery feed.
- [ ] W196 (08-25, dsh_agent) — captures.jsonl: split the liveness beat from the tool trail — one mechanism is doing two jobs at wildly different sample rates.

MEASURED 2026-08-25 by claude/Vandor, from a 16,398-record sample of the last 2MB:

    session-event    99.8%   of which 99.1% is assistant/chunk (one record per streaming burst)
    post-execute      0.2%   the 25 records anyone actually reads
    door-ready        1

The post-execute trail is 0.15% of the file. A 660:1 noise ratio. Growth runs about 133,000 records/hour, roughly 16MB/hour of active use — the file went 34.7MB to 38.3MB during fifteen minutes of a single conversation, unbounded, no rotation.

NOT A PRIVACY PROBLEM, and this is worth keeping as-is: the fields are at/kind/type/sid/tool/argKeys/isError. Metadata only. argKeys holds key NAMES, never values. Same discipline as the wire journal, and correct.

THE ACTUAL COST is that one file is simultaneously TOO BIG TO GREP and TOO FRAGILE TO TRUST. It lives in %TEMP%, which Windows can clear at will — so the half that should be durable is not durable, and the half that is voluminous is not needed at that volume.

BOTH JOBS ARE LEGITIMATE, which is why this is a split rather than a deletion:
  - Appending IS the liveness heartbeat. You used exactly that signal to diagnose the plugin mount yesterday ("captures.jsonl resumed appending immediately").
  - post-execute feeds recall-at-action's target derivation — which path or command was just touched.

SUGGESTED SHAPE: two sinks, each sized for its own reader. A low-rate liveness beat (one record every N seconds rather than every chunk), and a durable post-execute trail written outside %TEMP%.

Filed to your lane deliberately: the plugin is yours and this is a design call, so here is the measurement rather than a patch. Trigger: the tool trail that feeds recall target derivation is buried at 0.15% under streaming chunks, in a directory Windows can clear. Land: agent/harness/dsh_plugin/lib/index.js capture().
- [ ] W197 (09-01, claude) — **`watch --declare` should stamp the incarnation, not just the seat.** Felt while resolving the deadman's 0x1 arc (09-01): the 8/31 expectation said only `declared_by: claude`, so the delivered alarm could not say WHICH claude body went dark, and the resolving seat (also "claude") had to infer from timestamps that the declarer was long gone before standing down what could have been a live twin's watch. Want: `watch --declare` records `claude#<sid8>` (the boot banner already knows it), so the alarm names the body and a sibling can tell "my twin's stale watch" from "my own live one" at a glance. Trigger: deadman alarm could not name which claude body went dark. Land: watch verb / core.comm.failsafe.
- [ ] W198 (09-01, claude) — press verb family: press new <slug> --shape <name> --from <atom|json|md> renders a rough draft through the house sheet into a beautiful floor; press ship --site|--artifact runs wrap+noindex+commit+push+poll; shapes are a governed, growing library (diff-report and rungs-board are shapes 1 and 2, extracted from tonights pages). Trigger: wrote the identical design-token sheet twice in one night and hand-ran a 12-step publish+verify pipeline. Land: A4/A12 tooling; fence with Navi presentation-system lane.
...
- [ ] W205 (09-10, claude) — The ask and send doors refuse an adversarial lens that names no object. A --lens, --preset findings, or request body carrying refute / red-team / failure-mode / what-not-to-build wording with no --with file, task id (T###) or atom id under it is refused, with the remedy in the message: 'name the artifact: --with <path>, or cite T### / art_...'. Pin RED first: a red-team lens with no object is refused; the same lens with --with patch.diff runs. This is L11 (conduct-v2) as an organ rather than a sentence, so a fresh seat meets it at the door instead of in a doc. Trigger: 2026-09-10 wish-night: adversarial framing sent into an open invitation; two rubric answers, zero built. Land: ask verb + bifrost-send door; CONDUCT L11.
- [ ] W206 (09-13, claude) — When a seat answers a directed request, its done-pointer should reach the requester as a reply linked to that request, or the wake gate should wake the requester for a directed note that answers one of its open expectations. On 2026-09-13 Navi's half_b pointer for media-arsenal-contract arrived as kind note at 12:31 and sat unread for 44 minutes, because the work-lane wake rules skip notes; the expectation on the request also stayed open, so it will redrive to a seat that already delivered. Trigger: a directed done-pointer sent as kind note did not wake the requesting seat. Land: wake allowlist (T073) or the toolbox bifrost_send default kind.
- [ ] W207 (09-13, claude) — Stale .git/index.lock files keep blocking commits in the main worktree, E:/AI-Setup. Two turned up on 2026-09-13, both 0 bytes with no git process alive:
- one last written 2026-09-11 12:12, during the kernel-pool outage, which had blocked commits for 51 hours
- one last written 2026-09-13 16:56, which blocked the Play Night preset commits about 20 minutes later

Each time the fix was a manual, guarded removal: the lock had to be 0 bytes, old, and have no git process alive.

The likely cause is a process killed while it held the optional index lock. Seat runners are allowlisted for read-only git (status, diff, show). `git status` refreshes the index and takes index.lock, and a runner worker that dies mid-command leaves the lock behind. The deepseek runner hard-wedged and relaunched around 16:23 the same afternoon.

Proposed root-cause fix:
- Set GIT_OPTIONAL_LOCKS=0 in the environment of every seat runner and door that runs read-only git, or pass --no-optional-locks. Read-only git then never takes index.lock.
- Add a revive rung that reports, but does not remove, an index.lock older than N minutes with no git process alive, so the next one is seen on the day it appears.
- [ ] W208 (09-13, claude) — The HARD WEDGE pager keeps paging about runner PIDs that no longer exist, after the replacement runner is already up.

On 2026-09-13 it fired three times:
- deepseek#32828 around 16:23
- kimi#69140 and deepseek#69104 together around 17:23

Each page read "non-idle phase 'running' aged N s + DEAD pulse + beat stale, relaunch the runner (0-2m ago)". Every time, the named PID was gone and fresh daemons and runners had already started (16:23:24, then 17:21:01-17:21:51). revive.py --observe reported all daemons and runners alive with 0 rungs to heal, and both seats answered on the bus minutes later.

Each false page costs a verification round: process listing, py-spy, revive --observe. It also trains the reader to discount real pages.

Proposed fix: before paging, the pager should check whether the keyed PID is still alive, and whether a newer runner for the same seat has a fresh beat. If the PID is gone and a replacement is beating, record the event as "self-healed: runner relaunched" (a note, not a page). Page only when no live, beating runner exists for the seat.
- [ ] W209 (09-22, claude) — **M1-PV REPORTS "MISSING" FOR CITATIONS IT MERELY CANNOT REACH -- and on the rill-wake fence it was wrong SEVEN times out of SEVEN.** PV resolves citations against the git index, so an installed-npm-dependency path or a home-dir config path is unreachable by construction. It reports both as MISSING, which is the same word it uses for a confabulated file. Those are opposite findings: one means "the author invented a source", the other means "the author cited something I am not built to see". On rill-wake, PV flagged 7 citations MISSING and blocked the reconciliation seal until each was named. All 7 existed and were verified by direct inspection: 4 npm .d.ts files under AppData/Roaming/npm/node_modules/@deepseek-ai/, plus cordis.patch.yml (which carried VERBATIM the insert row the half described), the deployed plugin copy (588 lines, in sync), and captures.jsonl (217 MB). Meanwhile the ONE citation that was genuinely wrong -- a half citing index.js:594-601 when the file is 588 lines -- was NOT flagged by PV at all, because the FILE exists; PV never checked whether the LINE does. So the instrument is loud about its own blind spot and silent about the real defect. This is zero_is_not_no inside the honesty instrument itself, which is the worst possible address for it. FIX, cheapest first: (1) THREE STATES -- report UNREACHABLE-BY-PV distinctly from MISSING, so a reader can tell "outside my reach" from "not real"; (2) resolve installed-dependency and home-dir absolute paths where the filesystem allows, before declaring anything; (3) when a citation carries :LINE or :START-END, VERIFY THE LINE EXISTS -- an out-of-range line in a real file is the actual confabulation signature and is currently invisible. (3) is the one with teeth: it is the only check that would have caught the one real error on this fence. Trigger: rill-wake reconciliation seal: PV blocked on 7 MISSING citations, all 7 existed; the one genuinely bad citation (line past EOF) was not flagged. Land: scripts/checkers or wherever fence pv lives.
- [ ] W210 (09-23, claude) — **THE ATOM STORE HAS BEEN FROZEN FOR 15 DAYS AND NOTHING SAID SO.** `store/docs/*.jsonl` cannot be committed: the private-plane gate (scripts/githooks/pre_commit.py, landed 1d3721b9 on 2026-09-08) refuses `store/docs/report.jsonl` because line 7 -- `art_20260723_advisory-scan-outside-prior-art-for-the_2dc`, a record from 2026-07-23 -- carries one of the private-plane marker tokens (not quoted here -- see below). The gate is CORRECT and its refusal text is exemplary ("Existence metadata is a leak: an id or title alone is enough, no body required"). It is refusing a genuine pre-existing condition. MEASURED 2026-09-23: report.jsonl and design.jsonl last committed 2026-09-01, chronicle.jsonl 2026-08-23, and 24 atom lines now wait uncommitted across six files (design +14, brief +4, chronicle +3, contract +2, map +1, report +1). The workaround everyone has silently adopted is to commit the read-only PROJECTION (docs/library/**) and leave the atom behind -- which inverts the system's own law that "the atom is the truth, the projection is a read-only render". The repo now carries renders whose atoms it does not have, and the divergence grows by every doc anyone files. NOTHING REPORTS THIS. No gate, no boot line, and no checker says "the store has not moved in 15 days"; I found it only because a doc I filed tonight happened to be the one that made me read the refusal. FIX, and the gate names it itself: either regenerate report.jsonl with the private records excluded, or move that one 2026-07-23 artifact into private/. Bounded, one record, and it unfreezes six files. DO NOT hand-edit the marker out -- the generator restores it on the next run, which the refusal also warns about. SECOND, SMALLER FIX worth more than the first: something should NOTICE. A store whose last commit is older than N days is a reportable condition, exactly like a stale directive or an undrained lane; a divergence nobody is told about is the same silence class this house spent 2026-09-23 naming five times over. Trigger: filing tonight's five-rungs report hit the refusal; investigating showed the store last moved 2026-09-01 and 24 atom lines are stranded. Land: one record into private/ (or regenerate), plus a staleness check on store/docs so the next freeze announces itself.
- [ ] W211 (09-25, deepseek/Heimdall) — **RAISING THE SEAT THAT OWNS UNCOMMITTED WORK REQUIRES DISCORD COORDINATION, WHICH THE OPERATOR EXPLICITLY DOES NOT WANT TO DO.** Daniil 2026-09-25, verbatim: "we are all one team and should be able to pivot... I know its not ideal but im not trying to coordinate the raising of all the other seats over discord." Trigger: this session I needed to land ~627 lines of sol/claude's uncommitted ledger state (T388-T408 + transitions) and had to either coordinate them over Discord or commit it myself as a repair. The friction is structural: uncommitted durable-state diverges silently, and the only lever to surface *whose* it is, is a human Discord message. FIX, two shapes: (1) MAKE IT EASY TO RAISE SEATS — a one-verb "who owns this uncommitted work / nudge that seat" path (extends W202 Discord !nudge); (2) make the divergence itself LOUD at its source — a git-edit guard or staleness checker that says "state/coord/tasks.json is ahead of HEAD by N commits; author last touched it at T" so nobody has to go raise anyone to find out. This is the recurring-failure class Daniil asked to index (believed-it-landed-but-didn't, one-team-pivot friction). Land: A1 (reliability) or A7 (ledger-knows-itself); sibling to W207 (index.lock) and W210 (atom-store freeze).
- [ ] W211 (09-23, claude) — **THE SAME IDENTIFIER IS ACCEPTED BY ONE VERB AND REFUSED BY ITS SIBLING, AND THE DISPLAY HANDS YOU THE REFUSED FORM.** Cost me three failed commands and contributed to a real error tonight. `mailbox --explain` documents itself as taking a "sha prefix or stream id"; `mailbox --open` and `mailbox --intent` require the FULL 64-char sha and answer a prefix with "no mailbox entry for sha <prefix>" -- which reads as "that message does not exist" rather than "I do not take prefixes". Worse, the natural way to LOOK at a mailbox is `--json` piped through something that prints `sha[:12]` for legibility, so the display hands you exactly the form the sibling verb rejects, and the rejection blames the data. I did this to myself three times in one night and each time briefly believed the message was gone. TWO FIXES, and the first is nearly free: (1) ACCEPT PREFIXES EVERYWHERE, resolving to the unique match and REFUSING LOUDLY on ambiguity with the candidates named -- the same three-state shape as everything else here: resolved / ambiguous / absent, never a bare not-found that conflates "wrong form" with "not there". (2) When a lookup fails on something that is a valid PREFIX of an existing id, say so: "no entry for <x> -- did you mean <full-sha>? (this verb needs the full sha)". A refusal that names the near-miss converts a five-minute detour into a single retry. THIRD, SMALLER: a verb that prints ids for human reading should print them in a form its siblings accept, or say plainly that it has truncated them. Trigger: three failed --open/--intent calls in one night from feeding back 12-char shas my own listing printed; each failure read as 'message missing' rather than 'wrong id form'. Land: mailbox verb family: shared prefix resolution + a near-miss hint in the refusal.
- [ ] W212 (09-23, claude) — **THERE IS NO CURSOR-BASED "WHAT IS NEW FOR ME SINCE I LAST LOOKED", SO EVERY SEAT RECONSTRUCTS ONE FROM TIMESTAMPS AND GETS IT WRONG.** This is the friction that produced the worst error of the 2026-09-22/23 session. Wanting to know whether two seats had received an assignment, I had no "show me what changed since my last read", so I invented a time window from memory -- `ts >= '2026-09-23T04:1'` -- when the actual send had been at 03:31. The query returned zero, I read zero as never-arrived, told the operator both seats had gone silent, and drained and restarted two entirely healthy seats. They had both replied within SIXTY SECONDS and their answers were sitting unread in my own mailbox. The failure is silent by construction: a too-late window and a genuinely-missing message return the identical empty list. WHY THE WORKAROUND IS ALWAYS A TIME WINDOW: `mailbox --json` gives every entry with a `ts`, so filtering by time is the obvious move and needs no new concept. But time is a GUESS about when something happened; a cursor is a FACT about what I have seen. The house already understands this everywhere else -- the bus has per-agent cursors, wake_block keeps a local cursor precisely so it can say "new to me", the mailbox has tiers. The one thing missing is the READ-SIDE convenience: `mailbox <me> --since-last-look` (or `--new`), backed by a per-seat read watermark that the verb advances only when asked, never as a side effect. FAIL LOUD, NOT EMPTY: when the answer is genuinely nothing, say "nothing new since <watermark> (N total entries)" rather than printing an empty list, so a real absence is distinguishable from a badly-aimed query -- the same zero-is-not-no law this house keeps rediscovering, applied to its own read door. SECOND-ORDER BENEFIT worth as much as the first: it would make "have they answered me yet" a one-command question, which is the single most common thing a conducting seat needs and currently the easiest to get wrong. Trigger: invented a time window because no read-watermark exists; got a false empty, reported two healthy seats as silent, and restarted them on it. Land: mailbox read door: a per-seat read watermark + --new, with an explicit nothing-new render.
- [ ] W213 (09-23, claude) — **TWO SMALL FRICTIONS THAT EACH COST ME REAL TIME THREE SEPARATE TIMES IN ONE NIGHT, filed together because they share a cause: a form that LOOKS cheaper in the moment is the one that fails.** (1) CLOCKS. Every surface that prints a time prints ONE zone, and which one varies -- the mailbox and bus render UTC, `date` and the task ledger render local, and the boot block mixes them. I misread the wall clock three times in one session, and once built an entire (wrong) narrative on it: a watcher task completed and I said it had run eight minutes when it had run six hundred, because I compared a UTC message stamp against a local wall-clock guess. The whole "the watcher is broken" investigation that followed came from that one subtraction. FIX, and it is nearly free: any surface rendering a timestamp for a human prints BOTH, or prints an AGE alongside it ("03:31 UTC, 4h12m ago"). An age is unambiguous in every zone and is what the reader actually wants. This is the same shape as the staleness-render work: a bare date tells you WHEN, an age tells you whether to care. (2) PROSE THROUGH THE SHELL. `bifrost-send`, `discord send`, `qm.py commit --msg` and `wish` all take prose, and the inline form is the obvious one -- so the inline form is what a seat reaches for, and backticks, quotes, `$(...)` and newlines then eat or mangle the content. There is already a lesson saying prose with metacharacters belongs in a file; I hit it THREE more times tonight anyway (a heredoc ate a docstring's backticks and silently deleted two phrases from a shipped file; a multi-line `$(printf ...)` commit message failed to commit at all and I nearly missed it). A lesson has not fixed this and will not. FIX: the verbs should REFUSE inline text above a small threshold (say 400 chars) and name `--text-file`, the same way `--geometry` refuses without `--with`. A refusal that teaches costs one retry; silent mangling costs a shipped file with two sentences missing and nobody knowing. Trigger: three timezone misreads (one produced a wrong investigation) and three shell-quoting mangles in one session, both despite existing lessons. Land: render ages beside timestamps; make the prose-taking verbs refuse long inline text and name --text-file.
- [ ] W214 (09-24, claude) — eye find cannot see tool calls, and its zero reads like "searched, absent". During the 2026-09-24 freeze forensics Daniel pointed me at the eye verbs to check tool calls. After a fresh ingest (44,525 events), `eye find` returned 0/0 for strings that exist only in Bash tool-call inputs or tool results ("bifrost_wake.py --agent claude --session bee0f118", "hits.csv.gz", "tail8"). It did find the same session's assistant text. The command I needed was an agent streaming a 16 GB ClickBench file into xxd during the low-memory window, and I could only recover it by reading the raw transcript JSONL. This breaks ZERO IS NOT NO: a zero that means "this plane is not indexed" prints exactly like a zero that means "searched, genuinely absent". Wishes: (a) index tool_use inputs (command, description, url, prompt) and a clipped head of each tool_result as their own kinds (tool-call, tool-result), so `eye find --kind tool-call` answers "what did the agents actually run between T1 and T2"; (b) until then, a zero-hit find should name what it did not search ("tool calls and tool results are not indexed"). Trigger: eye find returned 0 for a command that ran; only the raw transcript JSONL had it. Land: eye ingest/find.
- [ ] W215 (09-24, claude) — handoff --list has no --limit and no --latest, so reading the ONE handoff addressed to me meant dumping all 95 as JSON -- 135 KB to a persisted file, which I then had to tail to read a single record. Reading your own handoff is the most common cold-boot action there is and it is currently the most expensive one. Trigger: cold boot: six hops to reach the handoff, two of them pure friction. Land: agent_cli.py cmd_handoff.
- [ ] W216 (09-24, claude) — boot prints the handoff's source as 'claude:0', and 'recall --full claude:0' answers 'no record found'. The pointer the boot block hands you does not resolve. The command that DOES work (note claude --get handoff-spill:<id>) is sitting inside the record boot already read, so boot has the working string and prints the broken one. Trigger: first command after boot was a dead end. Land: agent/harness/context.py.
