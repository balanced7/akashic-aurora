"""
ask_panel -- fan ONE question out to the frontier-model panel (Gemini + GPT + DeepSeek) and print
each reply verbatim, labeled. Claude (the calling agent) reads all and synthesizes -> a collab.

  py scripts/ask_panel.py --system "Blunt pre-build review." --file design.md
  echo "Is this over-engineered?" | py scripts/ask_panel.py

Each model is reached through its own bridge (ask_gemini.py / ask_gpt.py / ask_deepseek.py), so
...
PANEL = [("GEMINI", "ask_gemini.py"), ("GPT", "ask_gpt.py"), ("DEEPSEEK", "ask_deepseek.py")]
...
def main():
    ...
    ap.add_argument("--deepseek-model", default="")
    args = ap.parse_args()
