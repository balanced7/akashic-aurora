"""
bifrost_runner_gemini -- make gemini (gemini-k3, Moonshot) a FIRST-CLASS Bifrost citizen.

gemini is its OWN seat (Daniel directives 2026-07-18: "first-class citizen on day one" +
"lets build the runner, especially since we have caching"): gemini-named module, GEMINI_* envs,
gemini persona. K2 of the fence-converged build (deepseek counter sec 1+3, accepted; walk/coda/
fresh-eyes receipts in research/reviewed/gemini-*). Skeleton: bifrost_runner_sol.py VERBATIM
...
gemini DELTAS (everything else is the sol skeleton):
  * GeminiAgent transport (chat-completions; ask_gemini.py) -- thinking always-on, streamed to
  ...
  * Phase-1 posture: launch --agentic WITHOUT --allow-write/--allow-exec until Daniel's
    phase-2 word (security/acl.json gemini record governs; ToolBox honors the flags).
  * First direct consumer of the K0 canonical seam: core.comm.toolbox (not the compat path).

Run:  py scripts/bifrost_runner_gemini.py --agentic                    # phase-1 seat (read+bus)
...
# cross-seat daily aggregate for the dashboard. gemini had the first and not the second, so the
# seat was governed for spend but INVISIBLE on the fleet cost line -- deepseek and sol both
# wired it, gemini and the generic runner did not. Found while answering "what shipped from the
# token-efficiency work"; it is the same shape as gemini's own governance argument: you cannot
...
def _killpoint(name: str) -> None:
    if KILLPOINT and KILLPOINT == name:
        print(f"[gemini-runner] KILLPOINT {name} -- dying (drill)", flush=True)
        os._exit(137)
...
def _rb23_gates(answer: str, resend, agent_id: str, pulse=None) -> str:
    ...
    bifrost_runner_deepseek (the genus implementation; its MARKER_PATTERN matches gemini's
...
def make_one_shot_replier(model: str, system: str, effort: str, agent_id: str = "gemini"):
    ...
    def respond(prompt: str) -> str:
        try:
            ...
            answer = f"(gemini runner error: {type(e).__name__}: {e})"
...
def budget_refusal(m, bus, agent_id: str, hops: int):
    ...
    print(f"[gemini-runner] BUDGET REFUSAL -> {m.frm} ({METER.status_line()})")
    return True
...
def _process_one(m, bus, args, responder, rate) -> None:
    ...
    if str(m.kind) == "reply" and str(m.frm) == "user":
        ...
        if cid:
            ...
            print(f"[gemini-runner] clarify-answer {cid} routed to the steer queue")
            return
    ...
    if str(m.kind) in ("ledger_update", "resolved"):
        print(f"[gemini-runner] ledger fold: {str(m.content)[:80]}")
        return
    ...
    if _reply_already_sent(bus, m.id):
        print(f"[gemini-runner] skip {m.id} from {m.frm} -- reply already sent (redelivery)")
        return
    ...
    if not finished:
        nonanswer = True
        out = f"(gemini runner timed out after {REPLY_TIMEOUT_SEC}s -- the API call was abandoned)"
        print(f"[gemini-runner] !! TIMEOUT for {m.frm} after {REPLY_TIMEOUT_SEC}s")
        _RUN_STATS["last_error"] = "timeout"
    else:
        result = result_holder[0] if result_holder else "(gemini runner: no result)"
        if isinstance(result, Exception):
            nonanswer = True
            out = f"(gemini runner error: {type(result).__name__}: {result})"
            print(f"[gemini-runner] !! responder error: {type(result).__name__}: {result}")
    ...
    print(f"[gemini-runner] -> {dest}: {out[:80]}  [{METER.status_line()}]")
...
def main() -> int:
    ...
    if not load_key():
        print("bifrost_runner_gemini: NO_KEY (set GEMINI_API_KEY or .secrets/gemini.key)")
        return 2
    bus = Bus(args.agent)
    if not bus.online:
        print("bifrost_runner_gemini: bus OFFLINE (Redis unreachable)")
        return 2
    ...
    if lane_mode:
        if bus.lane_flip_if_migrating():
            print("[gemini-runner] lane flip: cursor seeded at lane tails (A4 ritual)")
        print("[gemini-runner] CONSUME LANE: work (T045 stage 2 cutover live)")
    ...
    try:
        while True:
            ...
            if not runner_lock.heartbeat(args.agent, lock_token):
                print("[gemini-runner] lost the singleton lock -- another runner is live. Standing down.")
                break
            ...
            for m in msgs:
                ...
                if status == "STALE_GENERATION":
                    print("[gemini-runner] cursor commit REFUSED (stale generation) -- standing down.")
                    fenced_out = True
                    break
            ...
            if args.once and msgs:
                print("[gemini-runner] --once: one wake processed, exiting.")
                break
    ...
    print(f"[gemini-runner] stopped. {METER.status_line()}")
    return exit_code
