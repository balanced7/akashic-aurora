# launch_kimi_libraryschema.ps1 -- headless kimi-k3 fresh-eyes round on the library-schema design.
# Brief: research/briefs/kimi-fresh-eyes-repo-filing-brief-2026-07-21.md (verbatim charter block).
# Env model cloned from scripts/local/launch_kimi_fresheyes.ps1 (smoke-proven 2026-07-18) plus the
...
Write-Host "[launcher] kimi library-schema headless session starting (brief $($brief.Length) chars)"
claude -p $brief `
  --allowedTools "Read" "Glob" "Grep" "Bash(py agent_cli.py *)" "Write(research/**)" "Edit(research/**)" "Write(scratch/**)" "Edit(scratch/**)" `
  --max-turns 80
$rc = $LASTEXITCODE
Write-Host "[launcher] kimi library-schema session exited rc=$rc"
exit $rc
