# launch_kimi_fresheyes.ps1 -- headless kimi-k3 fresh-eyes round on the packet-routing halves.
# Brief: research/briefs/kimi-fresh-eyes-packet-routing-brief-2026-07-18.md (protocol step 5).
# Env model cloned from scripts/local/launch_kimi_walk.ps1 (smoke-proven 2026-07-18) plus the
# headless launch discipline: AKASHIC_STOP_WAKE=0 (ephemeral-seat exemption, pinned by
...
$doc = Get-Content "E:\AI-Setup\research\briefs\kimi-fresh-eyes-packet-routing-brief-2026-07-18.md" -Raw
...
Write-Host "[launcher] kimi fresh-eyes headless session starting (brief $($brief.Length) chars)"
claude -p $brief `
  --allowedTools "Read" "Glob" "Grep" "Bash(py agent_cli.py *)" "Write(research/**)" "Edit(research/**)" "Write(scratch/**)" "Edit(scratch/**)" `
  --max-turns 80
$rc = $LASTEXITCODE
Write-Host "[launcher] kimi fresh-eyes session exited rc=$rc"
exit $rc
