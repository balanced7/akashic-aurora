...
# Brief: research/briefs/kimi-storm-clear-second-observer-brief-2026-07-21.md (night-run 2026-07-21).
...
$doc = Get-Content "E:\AI-Setup\research\briefs\kimi-storm-clear-second-observer-brief-2026-07-21.md" -Raw
...
Write-Host "[launcher] kimi storm-clear second-observer session starting (brief $($brief.Length) chars)"
claude -p $brief `
  --allowedTools "Read" "Glob" "Grep" "Bash(py agent_cli.py *)" "Write(research/**)" "Edit(research/**)" "Write(scratch/**)" "Edit(scratch/**)" `
  --max-turns 40
$rc = $LASTEXITCODE
Write-Host "[launcher] kimi storm-clear second-observer session exited rc=$rc"
exit $rc
