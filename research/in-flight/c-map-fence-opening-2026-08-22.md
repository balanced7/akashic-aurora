...
C of the enablement deck, per Daniil's sequencing ruling (verbatim: "C = the map
second (visibility while building; walks/trails/cases/drift rendered)"), with a
design fence wanted before build and Simon's eyes explicitly requested. The fence
produces a reconciled design; Daniil gates ratification and the better-auth
boundary call. Registration of the build's forecast rides T375 at approval.

## INPUTS (the evidence pack -- read these, cite these)

- This brief (P1–P6, Q1–Q5).
- `research/reviewed/the-eye-synthesis-heimdall-nversion-2026-08-22.md` -- the
  Eye end-to-end (LOD pyramid, grammar door, S0–S7 live in `core/eye/`).
- Note `simon-event-map-and-resources-2026-08-22` + the vandor-chat thread
  (Simon's AsyncAPI/fuma-docs/better-auth bid, verbatim).
- Ledger row T378 (trail sensing; substrate `recall:outcome`; gates G1/G2).
- Note `attention-sequence-2026-08-22` (the ruling) and docs/ROADMAP.md
  (primitives-once law).
- The live wire itself: `events:raw`, `bifrost:inbox:*` shapes (cite stream ids
  where you verify).

## THE QUESTION

One map or three? Design the map organ from the three convergent descriptions
(Simon's AsyncAPI event plane, the Eye LOD zoom axis, the T378 trail overlay):
what v1 shows, where it lives, its generation-stamp honesty pin, and what stays
off it.

## RULES OF ENGAGEMENT

- **Blind halves.** File your half without reading the other's; independence is
  the entire value (the door's authorship check enforces it). Reconciliation is
  where the halves meet, not before.
- **Evidence over vibes.** Claims carry VERIFIED / INFER / GUESS labels; VERIFIED
  cites file:line or a stream id. The operator's words ride verbatim, never
  paraphrased.
- **Attack the opening position.** P1–P6 are bids, not decisions -- refute with
  evidence and the kill is credited to you at reconcile (red is a gem).
- **One calibrated question back** per half if something blocks you; otherwise
  file what you can and name the gap.
- **Deadline:** two filed halves or 48h, whichever first; then Vandor reconciles
  and Daniil gates. File through the door: `py agent_cli.py fence write
  c-map-design --slot half_a|half_b --by <you>` (Simon's half may arrive as chat;
  it gets filed verbatim with --by simon attribution).
...
## OUTPUT CONTRACT (each half returns a DIFFERENT type -- convergence must be earned, not shared)

- **half_a (Heimdall) -- the MECHANISM half.** Numbered counters over P1–P5 with
  evidence labels (VERIFIED / INFER / GUESS, file:line where verified); the Eye
  zoom-seam integration contract at function granularity (what the generator
  calls, what the eye grammar exposes, what neither may know about the other);
  answers to Q1–Q5. Per major claim, a delta block: LOAD-BEARING / SENSITIVE /
  FIRST-SIGN / IGNORABLE. No spec prose -- mechanism only.
- **half_b (Simon) -- the SPEC half.** A skeleton AsyncAPI document covering ~5
  representative bus events (chat, handoff, reply-with-answers, ledger_update,
  trace) written against the real wire shapes; the fuma-docs + better-auth
  deployment sketch; prose answers to Q1 (front page) and Q2 (expressiveness).
  The artifact and its rationale -- no counters required.
- **reconciliation (Vandor).** Merged design citing both halves + the kill list
  (every claim a half refuted, credited to its finder), then Daniil's gate.
- **Both halves:** tag your load-bearing claims `V1..Vn` with a [VERIFIED/INFER/
  GUESS] tag per claim -- the tally verb aligns verdict lines across blind
  halves, and untagged claims cannot be aligned (fence-workspace first-cycle
  lesson).

## The joy line (design instruction, not decoration)

Daniil, 2026-08-22, verbatim: "Simon its fun watching you describe what I have
just been working on with Vandor from another direction!" -- this fence exists to
make that convergence LOAD-BEARING: three independent descriptions agreeing is
the evidence the organ is real; the design's job is to keep the three axes
independent enough that their agreement keeps meaning something.
