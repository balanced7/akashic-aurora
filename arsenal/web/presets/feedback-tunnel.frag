...
void main() {
  ...
  if (hasVideo) {
    ...
    float idle = 0.10 + 0.05 * sin(u_time * 0.5 + a * 2.0);
    ...
    core = vec3(0.12, 0.30, 0.55) * (ring * (0.45 + u_level * 2.0) + 0.18 * petal * (idle + spec))
  ...
  // beat punch: brighten on the beat (but keep the centre below full white)
