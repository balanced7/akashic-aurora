...
def test_b1_shape_catchable_cases_match():
    """RULED (kimi, 2026-07-28, r2-slice1-bar-ruling-kimi): SAFETY WINS. Clause 1 is
    >=3 -- specifically the set that survives the allowlist grammar -- and it is a
    FLOOR that rises WITH the grammar, never ahead of it. kimi's own words carry
    the contract: "a bar that demands more silence than the safe grammar can
    deliver is a bar that prices a wrong-silence as acceptable", and the forfeited
    three were never safely in the set (an interpreter, an arbitrary-code
    transform whose label was itself made on a clipped render, a knowledge-
    consumption read). The way to >=4 is an AST-verified read-only class -- the
