"""Pins for the sealed bridge envelope + chain (contract updated 2026-09-17 after adversarial
review: observe_in returns a verdict dict, `verified` is mandatory, unseal takes `me`, seal takes an
epoch — see tests/test_bridge_seal_hardening.py for why each of those changed)
...
def test_a_gap_is_reported_not_silently_tolerated(tmp_path):
    c = seal.Chain(tmp_path / "chain.json")
    assert c.observe_in("serge", seq=1, mid="m1", prev="", epoch="e1", verified=True)["missing"] == []
    assert c.observe_in("serge", seq=2, mid="m2", prev="m1", epoch="e1", verified=True)["missing"] == []
    gaps = c.observe_in("serge", seq=4, mid="m4", prev="m3", epoch="e1", verified=True)["missing"]
